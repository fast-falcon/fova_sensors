...
def _ensure_auth_in_config(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not sensor_cfg.get("auth_user") or not sensor_cfg.get("auth_pass"):
        ...
        save_config(cfg)
        print("[sensor_to_central] generated BasicAuth for sensor:", user)
    return cfg
...
def _get_central_public_key(host: str, port: int) -> Optional[bytes]:
    ...
    try:
        ...
        print("[sensor_to_central] error getting central public key:", e)
        return None

    pem = data.get("public_key")
    if not pem:
        print("[sensor_to_central] central /api/public_key no public_key field")
        return None
...
def _update_push_interval_from_central(
    ...
    try:
        ...
        print("[sensor_to_central] error getting push_interval:", e)
...
def _uplink_loop():
    ...
    if cfg.get("role") != "sensor":
        print("[sensor_to_central] role is not 'sensor' → exiting uplink loop")
        return
    ...
    print(
        f"[sensor_to_central] uplink loop started → central http://{central_host}:{central_http_port}, "
    ...
    while not _STOP_FLAG:
        try:
            ...
            if not pem:
                print("[sensor_to_central] no central public key, skip this round")
            else:
                ...
                try:
                    ...
                    if new_pi is not None:
                        try:
                            ...
                            print("[sensor_to_central] push_interval updated to", push_interval)
                        except Exception:
                            pass
                except (URLError, HTTPError) as e:
                    print("[sensor_to_central] HTTP error:", e)
                except Exception as e:
                    print("[sensor_to_central] error posting to central:", e)
        except Exception as e:
            print("[sensor_to_central] loop exception:", e)
...
if __name__ == "__main__":
    print("=== sensor_to_central test (dry run) ===")
    ...
    print("✅ sensor_to_central basic test (از نظر ساختار) OK")
